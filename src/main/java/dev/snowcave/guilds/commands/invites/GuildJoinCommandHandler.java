 ...
 */
public class GuildJoinCommandHandler extends GuildMemberCommandHandler {

    @Override
    public void handle(Player player, User user, String[] arguments) {
        if(arguments.length < 2){
            ChatUtils.send(player, ChatUtils.format("&b/guild join &e<&6Guild&e> &8- &7Invite a player to join your Guild."));
        } else {
            String guildName = String.join(" ", Arrays.copyOfRange(arguments, 2, arguments.length));
            ...
            if(guild.isPresent()){
                ...
                if(invite.isPresent() || guild.get().isPublic()){
                    if(guild.get().getLevel().getMaxMembers() > guild.get().getMembers().size()){
                        guild.get().getMembers().add(invite.get().getUser());
