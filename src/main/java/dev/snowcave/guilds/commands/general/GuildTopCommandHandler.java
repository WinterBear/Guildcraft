...
public class GuildTopCommandHandler implements GuildCommandHandler {
    ...
    public void handle(Player player, String[] arguments) {
        ...
        sortedGuilds.sort(Comparator.comparing(Guild::getRawLevel).thenComparing(Guild::getBalance));
        ...
        if(pages.size() > 0){
            ...
            for(Guild guild : pages.get(partition)){
                ChatUtils.send(player, "&6" + i++ + " &3" + guild.getGuildName() + " &8(&eLevel&7: &b" + guild.getRawLevel() + " &eMoney&7: &b" + guild.getBalance() + "&8)");
            }
        } else {
