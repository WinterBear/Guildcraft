...
public class GuildLevelCommandHandler extends GuildMemberPermissionCommandHandler {
    ...
    public void handleWithPermission(Player player, User user, String[] arguments) {
        ...
        if (!Levels.canUpgrade(guild.getLevel())){
            chatter.error("You are already at the max guild level! Congratulations!");
            ...
            if(!guild.getLevel().getGuildBonuses().isEmpty()){
            ...
            //Not enough currency
            chatter.error("You need " + Levels.getUpgradeCost(guild.getLevel()) + " Silver to purchase the next level for your guild.");
        }
    }
